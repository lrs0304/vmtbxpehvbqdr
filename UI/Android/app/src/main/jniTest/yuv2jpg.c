...
int WriteBitsStream(JPEGINFO *pJpgInfo,unsigned short value,BYTE codeLen,BYTE* pOut,int nDataLen)
{ 
	int posval;//bit position in the bitstring we read, should be<=15 and >=0 
	posval=codeLen-1;
printf("--------------------------------------------->pod:%d, value:%d\n",posval,value);
	while (posval>=0)
	{
		...
		posval--;
		pJpgInfo->bytepos--;
printf("-------------%ld-------------->bytepos:%d,---finish%d\n",sizeof(char),pJpgInfo->bytepos,pJpgInfo->bytepos-1);
...
int ProcessDU(JPEGINFO *pJpgInfo,float* lpBuf,float* quantTab,HUFFCODE* dcHuffTab,HUFFCODE* acHuffTab,short* DC,BYTE* pOut,int nDataLen)
{
	BYTE i    = 0;        
	unsigned int j    = 0;
	short diffVal = 0;              
...
int ProcessData(JPEGINFO *pJpgInfo,BYTE* lpYBuf,BYTE* lpUBuf,BYTE* lpVBuf,int width,int height,int myBufLen, int muBufLen,int mvBufLen,BYTE* pOut,int nDataLen)
{ 
...
{
	//printf("sizeof(JPEGAPP0)=%d\n",sizeof(JPEGAPP0));
	//printf("sizeof(JPEGDQT_8BITS)=%d\n",sizeof(JPEGDQT_8BITS));
	//printf("sizeof(JPEGSOF0_24BITS)=%d\n",sizeof(JPEGSOF0_24BITS));
	//printf("sizeof(JPEGDHT)=%d\n",sizeof(JPEGDHT));
	BYTE* pYBuf = in_Y;
	BYTE* pUBuf = in_U;
	BYTE* pVBuf = in_V;
	int nYLen = nStride  * height;
	//int nUVLen = nStride  * height / 2;
	int	nDataLen;
	JPEGINFO JpgInfo;
